...
export function NotificationsConfiguratiosReferencia(){
  ...
  const hooks = [
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="overflow-y-auto scrollbar-none">                      
          <div className="flex flex-col w-[30rem] h-32 ml-14 space-y-3"> 
            ...
            {hooks.map((hook) => (
               <div 
               key={hook}
               className="bg-[#F08F3E3D] w-[5.5rem] h-8 flex justify-center items-center rounded-[47px]"
        ...
        <Button type="submit" 
        className="bg-[#F08F3E] cursor-pointer w-32 h-11 text-[#ffffff] hover:bg-[#F08F3E] hover:text-[#ffffff] mt-12 ml-14">Configurar</Button>
        </form>
    )
}